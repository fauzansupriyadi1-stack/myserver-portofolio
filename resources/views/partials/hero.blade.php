{{-- ============================================================
     HERO SECTION — Clean Split Flexbox Layout
     Left Column: Text Content | Right Column: Portrait Photo
     ============================================================ --}}
...
<section id="hero" class="relative w-full overflow-hidden rounded-2xl sm:rounded-3xl flex flex-col bg-[#0d1f10] min-h-[88vh]">
    ...
    {{-- Background Alam --}}
    <img src="{{ asset('storage/nature_bg.jpg') }}"
         alt="Background"
         loading="eager"
    ...
    <nav class="relative w-full px-5 sm:px-7 lg:px-9 pt-6 sm:pt-7 z-30" x-data="{ open: false }">
        <div class="flex items-center justify-between bg-white/[0.08] backdrop-blur-md border border-white/10 rounded-full px-4 py-2.5 shadow-xl">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 shrink-0">
                ...
                <span class="text-white font-extrabold text-sm tracking-tight">{{ $settings['site_name'] ?? 'Fauzan.dev' }}</span>
            </a>
            ...
            <div class="hidden lg:flex items-center gap-1">
                ...
                <a href="{{ route('cv') }}" class="px-3 py-1.5 text-xs font-semibold rounded-full transition-all duration-200 text-white/60 hover:text-white hover:bg-white/10">
                    CV
                </a>
                <a href="{{ route('skills') }}" class="px-3 py-1.5 text-xs font-semibold rounded-full transition-all duration-200 text-white/60 hover:text-white hover:bg-white/10">
                    Skills
                </a>
                <a href="{{ route('certifications') }}" class="px-3 py-1.5 text-xs font-semibold rounded-full transition-all duration-200 text-white/60 hover:text-white hover:bg-white/10">
        ...
        <div x-show="open" x-transition class="mt-2 bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl p-3 flex flex-col gap-1">
            ...
            <a href="{{ route('cv') }}" @click="open=false" class="px-4 py-2.5 text-white/80 hover:text-white hover:bg-white/10 rounded-xl text-sm font-semibold transition-colors">CV</a>
            <a href="{{ route('skills') }}" @click="open=false" class="px-4 py-2.5 text-white/80 hover:text-white hover:bg-white/10 rounded-xl text-sm font-semibold transition-colors">Skills</a>
            <a href="{{ route('certifications') }}" @click="open=false" class="px-4 py-2.5 text-white/80 hover:text-white hover:bg-white/10 rounded-xl text-sm font-semibold transition-colors">
                <span x-show="lang === 'id'">Sertifikasi</span>
                <span x-show="lang === 'en'" x-cloak>Certifications</span>
            </a>
    ...
    <div class="relative z-10 flex-1 flex flex-col-reverse md:flex-row items-stretch justify-between w-full px-6 sm:px-12 lg:px-16 gap-8" style="min-height: 0;">
        ...
        <div class="w-full md:w-[52%] flex flex-col justify-center py-10 md:py-12">
            ...
            {{-- Description (Teks dirapikan warnanya ke text-white/80 agar sangat terlihat) --}}
            ...
            <div class="flex flex-wrap items-center gap-3 mb-10">
                <a href="{{ $hero?->primary_cta_url ?? '#contact' }}"
                ...
                <a href="{{ $hero?->secondary_cta_url ?? '#projects' }}"
            ...
            <div class="flex items-center gap-8 pt-6 border-t border-white/15">
                <div>
                    <div class="text-2xl font-black text-[#A3E635]">{{ $experiences_count ?? 0 }}</div>
                ...
                <div>
                    <div class="text-2xl font-black text-[#A3E635]">{{ $projects_count ?? $projects->count() }}</div>
                    <div class="text-xs font-semibold text-white/60 mt-1">
                ...
                <div class="w-px h-10 bg-white/15"></div>
                <div>
                    <div class="text-2xl font-black text-[#A3E635]">{{ $certifications_count ?? 0 }}</div>
                    <div class="text-xs font-semibold text-white/60 mt-1">
                ...
                <div class="w-px h-10 bg-white/15"></div>
                <div>
                    <div class="text-2xl font-black text-[#A3E635]">{{ $skills_count ?? 0 }}</div>
                    <div class="text-xs font-semibold text-white/60 mt-1">
        ...
        {{-- KANAN: Foto - Lebih ke tengah, ada spacing ke kiri --}}

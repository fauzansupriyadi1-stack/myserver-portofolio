{{-- ============================================================
     FAQ SECTION
     Alpine.js accordion with dynamic questions from DB.
     ============================================================ --}}
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div x-data="{ open: null }" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div class="border border-gray-100 rounded-2xl overflow-hidden hover:border-[#A3E635]/50 transition-colors duration-300"
                 :class="open === {{ $faq->id }} ? 'border-[#A3E635]/50 shadow-sm shadow-[#A3E635]/10' : ''">
                <button @click="open = open === {{ $faq->id }} ? null : {{ $faq->id }}"
                    ...
                    <span class="font-semibold text-[#111827] text-sm pr-4 leading-tight">{{ $faq->question }}</span>
                    <div class="shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300"
                         :class="open === {{ $faq->id }} ? 'bg-[#A3E635] rotate-45' : 'bg-gray-100'">
                        <svg class="w-4 h-4 transition-colors duration-300"
                             :class="open === {{ $faq->id }} ? 'text-[#1E3A2B]' : 'text-[#4B5563]'"
                ...
                <div x-show="open === {{ $faq->id }}"
                    ...
                    <p class="text-[#4B5563] text-sm leading-relaxed">{{ $faq->answer }}</p>
                </div>
            </div>
            @endforeach
        </div>
